...
// Fetch with retry and timeout when calling APIs (Hack Club AI, Amadeus Space)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const spaceUrl = process.env.AMADEUS_SPACE_URL;
    const hfToken = process.env.HF_TOKEN;
    const hackclubKey = process.env.HACKCLUB_AI_KEY;

    if (!spaceUrl || !hfToken) {
      console.error(
        "Server misconfigured: missing AMADEUS_SPACE_URL or HF_TOKEN",
      );
    ...
    console.log("FINAL MESSAGE TO AMADEUS:", finalMessage);
    console.log("USER FACTS TO AMADEUS:", userFacts);
    console.log("VERIFIED FACTS TO AMADEUS:", verifiedFacts);

    // Call Amadeus with 3 retry attempts
    ...
    try {
      ...
      console.error("All retry attempts failed reaching Amadeus Space:", e);
      return NextResponse.json({ reply: pickOfflineReply() });
    }
